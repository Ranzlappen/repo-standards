...
- [ ] **Branch-lifecycle discipline observed (v3.2).** During any AI-driven upgrade pass, sessions follow the branch-lifecycle rules (`PROMPT.md` rules 17–18): no commits are pushed to a branch whose PR has already merged (a fresh branch is cut from `origin/main` for new work — a merged PR cannot be reopened), and a working branch that has fallen behind `origin/main` is rebased onto it (not merged) before continuing.
