...
| **Signed releases via [sigstore](https://www.sigstore.dev/)** | live | The `release` job in [`auto-tag.yml`](./workflows/auto-tag.yml) calls [`actions/attest-build-provenance`](https://github.com/actions/attest-build-provenance) which signs a SLSA build-provenance attestation via Sigstore Fulcio (transparency log: Rekor) using GitHub OIDC. The Sigstore bundle is attached to every GitHub Release as `<tarball>.sigstore.json` and is also discoverable through the GitHub attestations API. Verify with `gh attestation verify <tarball> --owner Ranzlappen`. |
| **SLSA build provenance** | live | Generated for every tagged release by the same step above. Bundle lands on the Release alongside the source tarball + sha256, and is queryable via the GitHub attestations API. Downstream consumer pipelines remain responsible for their own SLSA posture; `templates/.github/workflows/release-please.yml` ships equivalent plumbing for npm / PyPI / GHCR publish paths. |
