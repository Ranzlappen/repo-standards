...
| **Signed releases via [sigstore](https://www.sigstore.dev/) / cosign** | live | The `provenance` job in [`auto-tag.yml`](./workflows/auto-tag.yml) calls the official [SLSA generic generator](https://github.com/slsa-framework/slsa-github-generator/) reusable workflow, which signs the provenance via Sigstore Fulcio (transparency log: Rekor) and attaches `.intoto.jsonl` to every GitHub Release. Verify with [`slsa-verifier verify-artifact --provenance-path <name>.intoto.jsonl --source-uri github.com/Ranzlappen/repo-standards <tarball>`](https://github.com/slsa-framework/slsa-verifier). |
| **SLSA build provenance — Level 3** | live | Generated for every tagged release by the same `provenance` job above (Level 3 isolation guarantees from the reusable-workflow trust model). Provenance lands on the Release alongside the source tarball + sha256. Downstream consumer pipelines remain responsible for their own SLSA posture; `templates/.github/workflows/release-please.yml` ships equivalent plumbing for npm / PyPI / GHCR publish paths. |
